...
const OneWorkSchedule = ( props: any ) => 
{
	...
	useEffect(() => 
	{
		if (isReady)
		{
			...
			{
				// Get record id
				let id = workRecords[i].id;
				recIds.push(id);

				// Get active status
				let active = workRecords[i].active;
				if (active === 0)
				{
    ...
    const handleSaveCheck = async (recId: number, isChecked: boolean) => 
    {
		...
		await DbSchedule.setWorkActive(recId, active)
		.then((result) => 
		{
		...
		if (active === 0)
		{
			await DbCalendar.clearRecords(1, recId)
			.then((result) => 
			{
				console.log('Result from clear records: ', result);
			});
			await DbSchedule.setWorkTimes(recId, 0, 0, 1)
			.then((result) => 
			{
				console.log('Result: ', result);
				setRefresh(!refresh);
    ...
    const handleSaveTime = async (index: number, recId: number, newTimes: StartEndTimes[]) => 
    {
		...
		await DbSchedule.setWorkTimes(recId, stTimestamp, etTimestamp, rollover)
		.then((result) => 
		{
			console.log('Result: ', result);
			setRefresh(!refresh);
...
export default OneWorkSchedule;